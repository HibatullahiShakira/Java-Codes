import java.util.Scanner;
	...
	public class MBTI {
		...
		String[][] questions = { {"A. expand energy, enjoy groups \tB. conserve energy, enjoy one-on-one", "E","I"}, 
		...
		for(int i = 0; i < questions.length; i++) {
			...
			System.out.println("Enter A or B as response");
			String response = scanner.nextLine().trim().toUpperCase();

			if ("A".equals(response) &&  "E".equals(questions[i][1]))  { scores[0]++;
			}else if ("B".equals(response) &&  "I".equals(questions[i][2]))  { scores[0]--;}
		
			if ("A".equals(response) &&  "S".equals(questions[i][1]))  { scores[1]++;
			}else if ("B".equals(response) &&  "N".equals(questions[i][2])) { scores[1]--;}

			if ("A".equals(response) &&  "T".equals(questions[i][1]))  { scores[2]++;
			}else if ("B".equals(response) &&  "F".equals(questions[i][2]))  { scores[2]--;}

			if ("A".equals(response) &&  "J".equals(questions[i][1]))  { scores[3]++;
			}else if ("B".equals(response) &&  "P".equals(questions[i][2]))  { scores[3]--;}
	...
	System.out.print("Your MBTI personality is " +  mbtiType);
